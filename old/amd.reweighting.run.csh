...
set amd_path = '/home/pmung/Dropbox/9_scripts/2_MD/amber/aMD'
...
# For AMBER14: 
# awk 'NR%1==0' gamd.log | awk '{print ($8+$7)/(0.001987*310)" " $2 " " ($8+$7)}' > gamd.weights.dat

# For AMBER12: 
# awk 'NR%1==0' amd.log | awk '{print ($8+$7)" " $3 " " ($8+$7)*(0.001987*300)}' > amd.weights.dat

# For NAMD simulation: 
# grep "ACCELERATED MD" namd.log | awk 'NR%1==0' | awk '{print $6/(0.001987*300)" " $4 " " $6 " "$8}' > amd.weights.dat
...
python $amd_path/amd.PyReweighting-1D.py \
  -input Psi.dat -cutoff 10 -Xdim -180 180 \
  -disc 6 -Emax 20 -job amdweight_CE \
  -weight amd.weights.dat | tee -a amd.reweight_variable.log
...
python $amd_path/amd.PyReweighting-1D.py \
  -input Psi.dat \
  -disc 6 -Emax 20 -job amdweight_MC \
  -order 10 -weight amd.weights.dat | tee -a amd.reweight_variable.log
mv -v pmf-Psi.dat.xvg pmf-Psi-reweight-MC-order10.xvg

# Reweighting using exponential average
python $amd_path/amd.PyReweighting-1D.py \
  -input Psi.dat \
  -disc 6 -Emax 20 -job amdweight \
  -weight amd.weights.dat | tee -a amd.reweight_variable.log
...
# Reweighting using cumulant expansion 
python $amd_path/amd.PyReweighting-2D.py \
  -cutoff 10 -input Phi_Psi -Xdim -180 180 \
  -discX 6 -Ydim -180 180 -discY 6 \
  -Emax 20 -job amdweight_CE \
  -weight amd.weights.dat | tee -a amd.reweight_variable.log
...
python $amd_path/amd.PyReweighting-2D.py \
  -input Phi_Psi \
  -Emax 100 -discX 6 -discY 6 -job amdweight_MC -order 10 \
  -weight amd.weights.dat | tee -a amd.reweight_variable.log
...
python $amd_path/amd.PyReweighting-2D.py \
  -input Phi_Psi \
  -Emax 20 -discX 6 -discY 6 -job amdweight \
  -weight amd.weights.dat | tee -a amd.reweight_variable.log
