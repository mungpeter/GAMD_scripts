...
###########MAIN
## This is modified from the original standalone gamd_reweight.py
## converted from python 2.6- to python 3.6+ compatible
## This is not used by the main shell script 2_gamd_reweight_run2d.csh
##
###############
def gamd_reweight_2d():
## Set control parameters
    ...
    data = loadfiletoarray(args.pwd, args.input, args.col)
...
def loadfiletoarray(pwd, files, col, skip=1):

    mpi = multiprocessing.Pool(processes = multiprocessing.cpu_count()/2)
    obj = LoadData(pwd=pwd, col=col, skip=skip)
...
class LoadData(object):
    ...
    def load_txt( self, infile ):
        load = np.loadtxt(self.pwd+'/'+infile, usecols=[int(self.col)-1])[::self.skip]
